...

{
    ...
    class SmoothScroll {
        constructor() {
            // the <main> element
            this.DOM = {main: document.querySelector('.jw_main_body')};
            // the scrollable element
            // we translate this element when scrolling (y-axis)
            this.DOM.scrollable = this.DOM.main.querySelector('[data-scroll]');
            // the items on the page
            this.items = [];
            [...this.DOM.main.querySelectorAll('.jw_content > .item')].forEach(item => this.items.push(new Item(item)));
    ...
    const preloadImages = () => {
        return new Promise((resolve, reject) => {
            ...
            imagesLoaded( 'body', function() {
                setTimeout(function() {
                    imagesLoaded(document.querySelectorAll('.item__img'), {background: true}, resolve);
                    jQuery('.jw_main_body main').css( 'padding-bottom', jQuery('footer').outerHeight() + 'px' );
                }, 1000);
            });
        });
    };
